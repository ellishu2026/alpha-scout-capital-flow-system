...
## Fixed List Moomoo Flow-Based Win Rate Recalculation

V1.9.8 adds `scripts/calculate_moomoo_flow_win_rate.py`, a research-only fixed-list pipeline that reads Moomoo archive rows for `SOXL`, `SMH`, `NVDA`, `MSFT`, `GOOGL`, `ORCL`, `RKLB`, `LLY`, and `IONQ`, computes Moomoo flow windows and flow signal groups, and joins them to archived OHLCV close prices when Supabase archive credentials are available.

The script writes `docs/research/moomoo-flow-win-rate-v1978.md`, `data/research/moomoo_flow_win_rate_v1978.json`, and `data/research/moomoo_flow_win_rate_v1978.csv`. In local environments without Supabase archive credentials, it falls back to the fixed-list XLSX flow import and marks forward-return samples `Not Ready` with missing price rows rather than fabricating returns. A lightweight `GET /api/research/moomoo-flow-win-rate` endpoint serves the generated JSON report. This is research only: no Entry / Position actions, scoring model, thresholds, rule promotion, Risk Gate behavior, provider priority, archive data, or trading functionality are changed.

## Fixed List Historical Close Price Archive for Forward Return

V1.9.9 adds `scripts/fetch_fixed_close_prices.py`, a fixed-list-only historical close price archive builder for `SOXL`, `SMH`, `NVDA`, `MSFT`, `GOOGL`, `ORCL`, `RKLB`, `LLY`, and `IONQ`. The script requests daily OHLCV data for `2026-01-01` through `2026-06-05` from Stooq daily CSV first and uses a yfinance-compatible fallback only when Stooq does not return usable rows. It writes `data/research/fixed_close_prices_v199.json` and `.csv` with source labels, close price type, fetched timestamp, and `BASIC_DAILY_OHLCV` quality metadata.

V1.9.9 updates the Moomoo fixed-list flow win-rate research to use the local close price archive when Supabase price archive rows are unavailable. The recalculation writes `docs/research/moomoo-flow-win-rate-v199.md`, `data/research/moomoo_flow_win_rate_v199.json`, and `.csv`, including price coverage, forward-return rows by horizon, missing future-price rows, ready-status counts, and best signal groups. Missing signal/future prices are excluded per horizon; no returns are fabricated.

The `/api/research/moomoo-flow-win-rate` endpoint now serves the V1.9.9 report and exposes `moomooFlowRows`, `priceRows`, `forwardReturnRows`, `metricsCount`, `bestSignalGroups`, and `readyStatusSummary` at top level. This remains research only: no Entry / Position actions, scoring model, threshold simulation, A/B logic, Rule Promotion Gate, Risk Gate behavior, fixed watchlist, Moomoo archive priority, provider ladder, or trading functionality is changed.

## Moomoo Flow Win Rate Result Review & Candidate Signal Selection

V2.0.0 adds `scripts/review_moomoo_flow_signal_candidates.py`, a research-only review layer on top of the V1.9.9 Moomoo flow win-rate outputs. It classifies every signal/horizon pair into Candidate, Watch, Risk / Reduce, Rejected, or Not Ready using sample size, win rate, average return, median return, profit factor, ticker concentration, and contributing ticker count guardrails.

The generated outputs are `data/research/moomoo_flow_signal_candidates_v200.json`, `data/research/moomoo_flow_signal_candidates_v200.csv`, and `docs/research/moomoo-flow-signal-candidates-v200.md`. The current review found 82 candidate signal/horizon pairs, 45 watch signals, 0 clean risk/reduce signals, and 3 rejected/not-ready signals. The strongest candidate families are persistent inflow and multi-window positive-flow agreement, including `flow3D > 0 and flow5D > 0 and flow10D > 0`, `flow10D > 0`, `flow5D percentile >= 70`, and `at least 7 positive days in latest 10D`.

V2.0.0 also adds `GET /api/research/moomoo-flow-signal-candidates` for the candidate summary and keeps `GET /api/research/moomoo-flow-win-rate` unchanged. The recommended next step is `V2.0.1 Flow Threshold Simulation`; no candidate is promoted to production in V2.0.0.

Version display is now guarded by `src/lib/version.ts`, with `APP_VERSION=V2.0.0` and `APP_VERSION_LABEL=AlphaScout Capital Flow System V2.0.0`. `GET /api/version` exposes the same source, and `scripts/check_version_consistency.py --expected V2.0.0` fails if visible dashboard/page version strings are stale. This version does not change Entry / Position actions, production scoring, thresholds, Rule Promotion Gate, A/B production logic, Risk Gate behavior, fixed watchlist, ranked candidate scoring, provider priority, archive behavior, or trading functionality.

## Moomoo Flow Research Rule Control Center Wiring

V2.0.1 wires the V2.0.0 Moomoo flow candidate review and V1.9.9 win-rate research outputs into the dashboard Rule Control Center, Forward Returns diagnostics, and Trade Win Rate Leaderboard. The page now displays real research values such as 82 candidates, 45 watch signals, 3 rejected/not-ready signals, 3,459 forward-return rows, 963 price rows, and 130 metrics instead of stale `0 / 30`, all-`N/A`, or generic `Not Ready` placeholders when the research data exists.

V2.0.1 adds `GET /api/research/rule-control-center`, which combines `moomoo_flow_signal_candidates_v200.json` and `moomoo_flow_win_rate_v199.json` into a research-only control payload. The Rule Control Center remains production locked: current production rule stays `V1.7.6 Production`, auto activation remains disabled, promotable count remains 0, and the promotion reason now correctly states that forward-return samples are available but threshold simulation and Risk Gate review are still required.
